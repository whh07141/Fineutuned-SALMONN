...
def prepare_multi_sample(wav_paths, wav_processor, cuda_enabled=True):
    """
    Prepare a single sample by concatenating multiple wav files with short silences,
    mirroring the multi-turn handling used during training.
    """
    ...
    # pad to at least 1s then truncate to 30s (consistent with training)
    ...
    spectrogram = wav_processor(audio_concat, sampling_rate=sr, return_tensors="pt")["input_features"]

    samples = {
        "spectrogram": spectrogram,
        "raw_wav": torch.from_numpy(audio_concat).unsqueeze(0),
        "padding_mask": torch.zeros(len(audio_concat), dtype=torch.bool).unsqueeze(0),
    }
    if cuda_enabled:
        samples = move_to_cuda(samples)
