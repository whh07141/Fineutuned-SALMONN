...
class Runner:
    ...
    def train_epoch(self, epoch):
        ...
        for i in metric_logger.log_every(range(self.iters_per_epoch), self.config.config.run.log_freq, header=header, logger=self.log_writter, start_step=epoch*self.iters_per_epoch):
            ...
            # detect invalid loss early and skip step
            if not torch.isfinite(loss):
                logging.warning(f"Non-finite loss detected at step {i}: {loss.item()}. Skipping update.")
            ...
            if (i + 1) % self.config.config.run.accum_grad_iters == 0:
                # gradient clipping
                max_norm = float(self.config.config.run.get("max_grad_norm", 1.0))
                if self.use_amp:
